...
## Latest Priority Update (2026-02-26)
- Completed: Telegram live voice path now uses faster-whisper in the real bridge STT execution path, with media-source final-text lock (`bridge_stt` only on media/inferred-media turns).
- Completed: Telegram runtime TTS path is now local-only piper in normal mode (no Deepgram TTS fallback), with dedicated policy smoke coverage.
- Completed: Installed OpenClaw relay preflight transcription call for audio media is now diagnostic-gated, so normal runtime no longer executes provider preflight transcript generation.
- Keep as P0 follow-up: upstream/reapply installed-runtime patches (embedded exec-guard, gateway/ws formatting, voice-call STT hints) after OpenClaw updates so behavior does not regress outside repo code.
- Keep as P1 follow-up: ensure live environments have `faster-whisper` runtime dependency installed and smoke parity remains strict (`deepgram` STT path not selected in normal Telegram media flow).
...
Status
- In progress: OpenClaw-only polling and bridge handlers are in place; transport-agnostic command/voice routing is centralized in `d_brain.integrations.openclaw_bridge`; production command MVP plus v1.1 UX polish (`/ping`, `/version`, `/voice on|off|status`), duplicate-update guard, TTS fallback observability, no-network E2E/health/fallback/ops smokes, and Windows ops recovery wrappers are available. v1.2 bridge preferences + reliability finish is implemented (`/prefs`, per-user `language_mode`/`voice_reply`/`brevity`, prefs-aware bridge voice behavior, sidecar timeout/degrade guard, prefs smokes). v1.3 bridge observability/admin controls are implemented (`/diag`, `/diag full`, runtime error counters/recent failures, normalized structured bridge/handler logs, diagnostics smokes). v1.4 RC hardening pass completed for timeout/log sanitization, command safety guards, unified no-polling RC smoke suite, and Telegram voice-note STT observability hardening (stable fallback taxonomy, expanded structured evidence fields, operator log-summary helper, edge smokes). Telegram bridge STT runtime path is now local faster-whisper only (Deepgram STT removed from runtime bridge voice execution), with internal decode for compressed media and multipass diagnostics. A temporary local OpenClaw runtime exec-guard hotfix is applied in installed `pi-embedded` bundles to rewrite embedded direct Deepgram audio STT `exec` calls to the project redirect shim, with broader Deepgram signature matching and default wrapper-first/fail-closed behavior (direct Deepgram allowed only by explicit diagnostic override `OPENCLAW_AUDIO_DIRECT_DEEPGRAM_DIAGNOSTIC`). The wrapper-first embedded transcript path is UTF-8 hardened (`TranscriptB64` support, explicit empty/corrupted transcript diagnostics, mojibake suppression before adapter fallback input), and the strict live verdict helper now supports runtime-style request IDs (`oc-...|fc-...`) plus nested OpenClaw `raw` log payload markers while keeping same-requestId full-chain confirmation strict. A temporary local Windows `gateway/ws` pretty-log mojibake mitigation is also applied in installed `push-apns` bundles (ASCII-safe symbols + sanitized file writes). A temporary local `voice-call` streaming STT hardening patch is also applied in the installed `extensions/voice-call` runtime (OpenAI Realtime STT language + anti-transliteration prompt + transcript diagnostics) to address RU/mixed RU+EN transliteration bias. Remaining work is MSI end-to-end live verification of the embedded path (single fresh same-requestId full-chain proof + no direct Deepgram session evidence + mixed RU/EN transcript quality), live verification of post-patch `gateway/ws` file-log output after restart, live voice-call RU/EN/mixed STT verification for the new `voice-call` patch, upstreaming/persisting the local runtime fixes, updating legacy voice dispatch smoke cases for faster-whisper decode-first runtime assumptions, plus OpenClaw cron/system hook wiring to `d_brain.integrations.openclaw_jobs.run_job(...)` and runtime sender registration on startup.
